...
def mountImage(targetDir):
    loopFile = subprocess.check_output(['bash', '-c', 'source config && add_partition %s/image.raw' % targetDir]).decode().strip()
...
def umountImage(targetDir, loopFile):
    ...
    subprocess.check_output(['bash', '-c', 'source config && del_partition %s' % loopFile.rsplit('p', 1)[0]])
